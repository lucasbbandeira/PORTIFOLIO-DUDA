(() => {
  ...
  const header = () => `<header class="site-header"><a class="site-brand" href="index.html">DIAARIO DA DUDA<span>.</span></a><nav class="site-nav" aria-label="Navigation">${nav.map(([href, label]) => `<a href="${href}">${t(label)}</a>`).join("")}</nav><div class="header-tools"><div class="language-switcher" aria-label="${t(T("Idioma", "Language", "Idioma"))}">${["pt", "en", "es"].map((code) => `<button type="button" data-language="${code}" class="${language === code ? "is-active" : ""}" aria-pressed="${language === code}">${code.toUpperCase()}</button>`).join("")}</div><button class="theme-toggle" type="button" aria-label="${t(T("Alternar tema", "Toggle theme", "Cambiar tema"))}">${document.documentElement.dataset.theme === "dark" ? "☀" : "☾"}</button><button class="menu-button" type="button" aria-label="${t(T("Abrir menu", "Open menu", "Abrir menú"))}"><span></span><span></span></button></div></header>`;
  const footer = () => `<footer class="site-footer"><div><p class="footer-mark">DIAARIO DA DUDA.</p><p>${t(T("Viagens, moda, hospitalidade e conteúdo que conecta.", "Travel, fashion, hospitality and content that connects.", "Viajes, moda, hospitalidad y contenido que conecta."))}</p></div><div class="footer-contact"><span>${t(T("Contato profissional", "Professional contact", "Contacto profesional"))}</span><a href="mailto:dudabb2003@gmail.com">dudabb2003@gmail.com</a></div>${social()}<p class="footer-legal">© 2026 DIAARIO DA DUDA. ${t(T("Todos os direitos reservados.", "All rights reserved.", "Todos los derechos reservados."))}</p></footer>`;

  function renderHome() {
    const universes = [
      ["viagens.html", "travel", "01", T("Viagens", "Travel", "Viajes"), T("Roteiros, destinos e experiências vividas pelo mundo.", "Routes, destinations and experiences lived around the world.", "Rutas, destinos y experiencias vividas alrededor del mundo.")],
      ["moda.html", "fashion", "02", T("Moda", "Fashion", "Moda"), T("Looks de inverno, arrume-se comigo e estilo em viagem.", "Winter looks, get-ready-with-me videos and travel style.", "Looks de invierno, arréglate conmigo y estilo de viaje.")],
      ["ugc.html", "ugc", "03", T("UGC", "UGC", "UGC"), T("Conteúdo autêntico criado para marcas e experiências.", "Authentic content created for brands and experiences.", "Contenido auténtico creado para marcas y experiencias.")],
      ["hospitalidade.html", "hospitality", "04", T("Hospitalidade", "Hospitality", "Hospitalidad"), T("Hospedagens, gastronomia e experiências que valem a viagem.", "Stays, gastronomy and experiences worth the journey.", "Alojamientos, gastronomía y experiencias que valen el viaje.")],
    ];
    return `<section class="home-hero"><img src="assets/images/duda-neve.png" alt="Duda em uma paisagem de neve"><div class="home-hero-shade"></div><div class="home-hero-copy"><p class="eyebrow">${t(T("Viagens · Moda · UGC · Hospitalidade", "Travel · Fashion · UGC · Hospitality", "Viajes · Moda · UGC · Hospitalidad"))}</p><h1>DUDA</h1><strong>${t(T("Criadora de conteúdo & narradora visual", "Content creator & visual storyteller", "Creadora de contenido y narradora visual"))}</strong><p>${t(T("Experiências vividas transformadas em histórias visuais para pessoas, destinos e marcas.", "Lived experiences transformed into visual stories for people, destinations and brands.", "Experiencias vividas transformadas en historias visuales para personas, destinos y marcas."))}</p><div class="hero-actions"><a class="button button--light" href="#portfolio">${t(T("Ver portfólio", "View portfolio", "Ver portafolio"))} ↓</a><a class="button button--ghost" href="sobre.html">${t(T("Sobre a Duda", "About Duda", "Sobre Duda"))} ↗</a></div></div></section>
      ...
      <section class="partners-band">${heading(T("UGC & colaborações", "UGC & collaborations", "UGC y colaboraciones"), T("Marcas que já fizeram parte do diário", "Brands already featured in the journal", "Marcas que ya formaron parte del diario"))}<div class="brand-row"><img src="assets/brands/fotoregistro.png" alt="FotoRegistro"><img src="assets/brands/tour-curitiba.svg" alt="Tour Curitiba"><img src="assets/brands/real-seguro.svg" alt="Real Seguro Viagem"></div><a class="text-link" href="ugc.html">${t(T("Conhecer o UGC", "Explore UGC", "Conocer el UGC"))} ↗</a></section>
  ...
  function renderUGC() {
    const chip = ["omeuchip", "affiliate/omeuchip", T("O Meu Chip · Afiliada", "O Meu Chip · Affiliate", "O Meu Chip · Afiliada"), T("Conectividade para a próxima viagem", "Connectivity for the next journey", "Conectividad para el próximo viaje")];
    const brands = [
      ["FotoRegistro", "assets/brands/fotoregistro.png", T("Produtos personalizados, recebidos e álbum de viagem apresentados em conteúdos reais.", "Personalized products, an unboxing and a travel album featured in real content.", "Productos personalizados, unboxing y álbum de viaje presentados en contenido real.")],
      ["Tour Curitiba", "assets/brands/tour-curitiba.svg", T("Três experiências em Curitiba transformadas em vídeos para aproximar público e destino.", "Three Curitiba experiences transformed into videos that connect audiences with the destination.", "Tres experiencias en Curitiba transformadas en videos que acercan al público con el destino.")],
      ["Real Seguro Viagem", "assets/brands/real-seguro.svg", T("Parceria confirmada na trajetória profissional da Duda.", "A confirmed partnership in Duda’s professional journey.", "Una colaboración confirmada en la trayectoria profesional de Duda.")],
    ];
    return `${pageHero(T("Conteúdo feito para conectar", "Content made to connect", "Contenido creado para conectar"), T("UGC", "UGC", "UGC"), T("Vídeos com linguagem próxima, contexto real e uma narrativa pensada para aproximar marcas e pessoas.", "Videos with a relatable voice, real context and storytelling designed to connect brands with people.", "Videos con un lenguaje cercano, contexto real y una narrativa pensada para acercar marcas y personas."))}
      <section class="partnership-grid-section">${heading(T("UGC & colaborações", "UGC & collaborations", "UGC y colaboraciones"), T("Marcas que fazem parte do diário", "Brands featured in the journal", "Marcas que forman parte del diario"), T("Conteúdos e relações comerciais reunidos em uma única página.", "Content and commercial relationships brought together on one page.", "Contenidos y relaciones comerciales reunidos en una sola página."))}<div class="partnership-grid">${brands.map(([name, logo, description], index) => `<article class="partnership-card"><span>0${index + 1} · ${t(T("MARCA NO PORTFÓLIO", "BRAND IN THE PORTFOLIO", "MARCA EN EL PORTAFOLIO"))}</span><div class="partnership-logo"><img src="${logo}" alt="${name}"></div><h2>${name}</h2><p>${t(description)}</p></article>`).join("")}</div></section>
      <section class="brand-story brand-story--foto"><div class="brand-story-heading"><img src="assets/brands/fotoregistro.png" alt="FotoRegistro"><div><p class="eyebrow">${t(T("Parceria confirmada · FotoRegistro", "Confirmed partnership · FotoRegistro", "Colaboración confirmada · FotoRegistro"))}</p><h2>${t(T("Memórias que viraram conteúdo", "Memories transformed into content", "Recuerdos transformados en contenido"))}</h2></div></div>${gallery(foto)}</section>
      <section class="brand-story brand-story--tour"><div class="brand-story-heading"><img src="assets/brands/tour-curitiba.svg" alt="Tour Curitiba"><div><p class="eyebrow">${t(T("Parceria confirmada · Tour Curitiba", "Confirmed partnership · Tour Curitiba", "Colaboración confirmada · Tour Curitiba"))}</p><h2>${t(T("Curitiba vivida por experiências", "Experiencing Curitiba", "Curitiba vivida a través de experiencias"))}</h2></div></div>${gallery(tour)}</section>
      <section class="affiliate-section affiliate-section--ugc"><div><p class="eyebrow">O MEU CHIP · ${t(T("AFILIADA", "AFFILIATE", "AFILIADA"))}</p><h2>${t(T("Conexão para a próxima viagem.", "Stay connected on your next journey.", "Conéctate en tu próximo viaje."))}</h2><p>${t(T("Use o cupom da Duda e receba 15% de desconto em conectividade internacional.", "Use Duda’s code for 15% off international connectivity.", "Usa el cupón de Duda y obtén un 15% de descuento en conectividad internacional."))}</p><strong>DIAARIODADUDA</strong><a class="button" href="https://omeuchip.com/?ref=MARIAEDUARDA" target="_blank" rel="sponsored noreferrer">${t(T("Usar 15% de desconto", "Use the 15% discount", "Usar 15% de descuento"))} ↗</a></div>${gallery([chip])}</section>
      ${cta(T("Conteúdo real para marcas que querem ser lembradas.", "Authentic content for brands that want to be remembered.", "Contenido auténtico para marcas que quieren ser recordadas."))}`;
  }
  ...
  function render() {
    const page = document.body.dataset.page || document.querySelector("[data-site-page]")?.dataset.sitePage || "home";
    const pageRenderer = pageRenderers[page] || renderHome;
    document.getElementById("app").innerHTML = `${header()}<main>${pageRenderer()}</main>${footer()}`;
    bindEvents();
  }
